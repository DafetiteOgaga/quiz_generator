...
def main():
	...
	head_str = "Enter a heading for the quiz: "
	...
	try:
		...
		quizQuestionFilePath = question_location
		quizAnswerFilePath = answer_location
	...
	for quizFileNumber in range(number_of_types):
		type1 = quizFileNumber + 65
		...
		answerQuizFileName = open(f"{quizAnswerFilePath}{delimiter}{file_type}", "w")
		answerQuizFileName.write(textAnswers + body)

        # questions txtfile creation and modification
		questionQuizFileName = open(f"{quizQuestionFilePath}{delimiter}{file_type}", "w")
		questionQuizFileName.write(text + bio)
		...
		for quizNum in range(number_of_QA):
			# creation of correct answer for every question 
			correctAnswer = questions_and_answers_var[selectedStates[quizNum]]
